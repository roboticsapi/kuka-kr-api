...
public abstract class AbstractKR extends AbstractRobotArm<Joint, RobotArmDriver> implements DHRobotArm {
	...
	/*
	 * When the wrist root point, located at the intersection of axes A4, A5 and A6,
	 * has negative x coordinates in the coordinate system of the axis 1 (inclusive
	 * given joint rotation), this is a overhead solution. Otherwise it's a basic
	 * solution. This function assumes, the joint 5 is the wrist point and works
	 * probably only with KUKA KR robots.
	 */
	public boolean isOverheadSolution(double[] jointValues) throws SensorReadException, TransformationException {
		Map<Relation, RelationSensor> jointMap = this.getForwardKinematicsRelationMap(jointValues);

		Transformation A1toA5 = getJoint(0).getMovingFrame().getTransformationTo(getJoint(5).getFixedFrame(), true,
				jointMap);
